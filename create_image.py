...
class create_image:
    def __init__(self):
        self.type_image=int('Please input to choose type of molecular to take photos:\n 0: \t 2D;\n 1: \t 3D_cube;\n 2: \t 3D_tetrahedron')
        self.ligand_dir=input('Please input the path of location to take pdb files or sdf files')
        self.png_dir=input('Please input the path of location to save image')
        self.ligand_name=[]
        for i in sorted(glob.glob(self.ligand_dir+"/*.pdb")):
            self.ligand_name.append(i[len(ligand_dir)+1:-4])

    def png_mol_2D(self,a):
        cmd.delete('all')
        command = "load "+ligand_dir+"/"+a+".sdf"
        cmd.do(command)
    ...
    def png_mol_3D_cube(self,a):
        ...
        cmd.do("color cyan, (elem Cl)")
        cmd.do("color deepsalmon, (elem F)")
        command = png_dir+"/"+i+"_1.png"
        cmd.png(command, width=300, height=300, dpi=600)
        cmd.rotate("x","90")
        cmd.zoom(a,2)
        command = png_dir+"/"+i+"_2.png"
        cmd.png(command, width=300, height=300, dpi=600)
        cmd.rotate("x","90")
        cmd.zoom(a,2)
        command = png_dir+"/"+i+"_3.png"
        cmd.png(command, width=300, height=300, dpi=600)
        cmd.rotate("x","90")
        cmd.zoom(a,2)
        command = png_dir+"/"+i+"_4.png"
        cmd.png(command, width=300, height=300, dpi=600)
        cmd.rotate("x","90")
        cmd.rotate("y","90")
        cmd.zoom(a,2)
        command = png_dir+"/"+i+"_5.png"
        cmd.png(command, width=300, height=300, dpi=600)
        cmd.rotate("y","180")
        cmd.zoom(a,2)
        command = png_dir+"/"+i+"_6.png"
        cmd.png(command, width=300, height=300, dpi=600)
        cmd.rotate("y","90")
        cmd.delete(a)
    ...
    def fit(self):
        ...
        if self.type_image==0:
            for i in self.ligand_name:
                self.png_mol_2D(i)

        elif self.type_image==1:
            for i in self.ligand_name:
                self.png_mol_3D_cube(i)


        elif self.type_image==2:
            for i in self.ligand_name:
                self.png_mol_3D(i)
